...
+ (NSString *) currentLocalizedDeviceModel {
#if TARGET_OS_IPHONE
...
+ (BMFDeviceOrientation) currentDeviceOrientation {
	...
	if (orientation==UIInterfaceOrientationLandscapeLeft) return BMFDeviceOrientationLandscapeLeft;
	else if (orientation==UIInterfaceOrientationLandscapeRight) return BMFDeviceOrientationLandscapeRight;
	else if (orientation==UIInterfaceOrientationPortrait) return BMFDeviceOrientationPortrait;
	else if (orientation==UIInterfaceOrientationPortraitUpsideDown) return BMFDeviceOrientationPortraitUpsideDown;
	else return BMFDeviceOrientationUnknown;
#else
...
+ (BOOL) orientationIsLandscape:(BMFDeviceOrientation)orientation {
	return (orientation==BMFDeviceOrientationLandscapeLeft || orientation==BMFDeviceOrientationLandscapeRight);
}

+ (BOOL) orientationIsPortrait:(BMFDeviceOrientation)orientation {
	return ![self orientationIsLandscape:orientation];
}

+ (BOOL) inLandscape {
	return [self orientationIsLandscape:[self currentDeviceOrientation]];
}

+ (BOOL) inPortrait {
	return [self orientationIsPortrait:[self currentDeviceOrientation]];
}
