...
- (instancetype) initWithViewController:(UIViewController * __nonnull) viewController {
	BMFAssertReturnNil(viewController);
	self = [super init];
	if (self) {
...
- (NSArray *) emails {
	if (self.to.count>0) return self.to;
...
- (void) run:(BMFCompletionBlock)completionBlock {
	...
	if (self.openExternalApp || !canSendMail) {
...
//		BOOL result = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:finalUrlString]];
		...
		for (NSURL *url in self.attachedFileUrls) {
			BMFAssertReturn([url isKindOfClass:[NSURL class]]);
			NSData *fileData = [NSData dataWithContentsOfURL:url];
			
			NSString *mimeType = [BMFUtils mimeTypeForFileUrl:url];
			if (!mimeType) {
				mimeType = @"application/octet-stream";
			}
			if (fileData) [self.emailVC addAttachmentData:fileData mimeType:mimeType fileName:[url lastPathComponent]];
		}
		
		if (completionBlock) self.completionBlock = completionBlock;

		if (self.viewController.navigationController) {
			[self.viewController.navigationController presentViewController:self.emailVC animated:YES completion:nil];
		}
		else {
...
-(void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
	...
	else
	{
		...
		if (self.completionBlock) {
			if (result==MFMailComposeResultCancelled) self.completionBlock(nil,[NSError errorWithDomain:@"email" code:BMFErrorCancelled userInfo:nil]);
			else self.completionBlock(@(result),error);
		}
		self.completionBlock = nil;
	}
}
