...
+ (NSArray *) fill:(BMFIXView *)parent with:(BMFIXView *) view margin:(CGFloat) margin {
	return [self fill:parent with:view margin:margin priority:BMFLayoutPriorityRequired];
}

+ (NSArray *) fill:(BMFIXView *)parent with:(BMFIXView *) view margin:(CGFloat) margin priority:(BMFLayoutPriority) priority {
	return @[
			 [self fillHorizontally:parent withViews:@[view] margin:margin priority:priority],
 			 [self fillVertically:parent withViews:@[view] margin:margin priority:priority]
			 ];
}

+ (NSArray *) fillHorizontally:(BMFIXView *)parent withViews:(NSArray *)views margin:(CGFloat) margin {
	return [self fillHorizontally:parent withViews:views margin:margin priority:[self defaultLayoutPriority]];
}

+ (NSArray *) fillVertically:(BMFIXView *)parent withViews:(NSArray *) views margin:(CGFloat) margin {
	return [self fillVertically:parent withViews:views margin:margin priority:[self defaultLayoutPriority]];
}
...
+ (NSArray *) center:(NSArray *) views inParent:(BMFIXView *)parent margin:(CGFloat) margin {
	return @[
			 [BMFAutoLayoutUtils centerVertically:views inParent:parent margin:0],
			 [BMFAutoLayoutUtils centerHorizontally:views inParent:parent margin:0]
			 ];
}
...
+ (NSArray *) distributeHorizontally: (NSArray *)views inParent:(BMFIXView *)parent margin:(CGFloat) margin {
	return [self distributeHorizontally:views inParent:parent internalMargin:margin externalMargin:margin];
}
...
+ (NSArray *) distributeHorizontally: (NSArray *)views inParent:(BMFIXView *)parent internalMargin:(CGFloat)internalMargin externalMargin:(CGFloat)externalMargin {
	return [self distributeHorizontally:views inParent:parent internalMargin:internalMargin externalMargin:externalMargin priority:[self defaultLayoutPriority]];
}
...
+ (NSArray *) distributeHorizontally: (NSArray *)views inParent:(BMFIXView *)parent internalMargin:(CGFloat)internalMargin externalMargin:(CGFloat)externalMargin priority:(BMFLayoutPriority)priority {
	...
	[views enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
		...
		if (idx==0) {
			NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:parent attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeLeft multiplier:1 constant:-externalMargin];
			constraint.priority = priority;
			[results addObject:constraint];
		}
		else {
			BMFIXView *previous = views[idx-1];
			NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:previous attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeLeft multiplier:1 constant:-internalMargin];
			constraint.priority = priority;
			[results addObject:constraint];
		}
		
		if (idx==views.count-1) {
			NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:parent attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeRight multiplier:1 constant:externalMargin];
			constraint.priority = priority;
			[results addObject:constraint];
...
+ (NSArray *) distributeVertically: (NSArray *)views inParent:(BMFIXView *)parent margin:(CGFloat) margin {
	return [self distributeVertically:views inParent:parent internalMargin:margin externalMargin:margin];
}
...
+ (NSArray *) distributeVertically: (NSArray *)views inParent:(BMFIXView *)parent internalMargin:(CGFloat)internalMargin externalMargin:(CGFloat)externalMargin {
	return [self distributeVertically:views inParent:parent internalMargin:internalMargin externalMargin:externalMargin priority:[self defaultLayoutPriority]];
}
...
+ (NSArray *) distributeVertically: (NSArray *)views inParent:(BMFIXView *)parent internalMargin:(CGFloat)internalMargin externalMargin:(CGFloat)externalMargin priority:(BMFLayoutPriority)priority {
	...
	[views enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
		...
		if (idx==0) {
			NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:parent attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeTop multiplier:1 constant:-externalMargin];
			constraint.priority = priority;
			[results addObject:constraint];
		}
		else {
			...
			NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:previous attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeTop multiplier:1 constant:-internalMargin];
			constraint.priority = priority;
			[results addObject:constraint];
		}
		
		if (idx==views.count-1) {
			NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:parent attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeBottom multiplier:1 constant:externalMargin];
			constraint.priority = priority;
			[results addObject:constraint];
		}
	}];
...
#pragma mark Proportion
+ (NSLayoutConstraint *) makeSquare:(BMFIXView *) view {
	return [self proportionEqual:view multiplier:1 priority:[self defaultLayoutPriority]];
}

+ (NSLayoutConstraint *) proportionEqual:(BMFIXView *) view multiplier:(CGFloat) multiplier {
	return [self proportionEqual:view multiplier:multiplier priority:[self defaultLayoutPriority]];
}

+ (NSLayoutConstraint *) proportionEqual:(BMFIXView *) view multiplier:(CGFloat) multiplier priority:(BMFLayoutPriority) priority {
	return [self proportionEqual:view multiplier:multiplier constant:0 priority:priority];
}
...
+ (NSLayoutConstraint *) proportionEqual:(BMFIXView *) view multiplier:(CGFloat) multiplier constant:(CGFloat) constant priority:(BMFLayoutPriority) priority {
	NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeHeight multiplier:multiplier constant:constant];
	constraint.priority = priority;
	[view addConstraint:constraint];
	return constraint;
}

#pragma mark Equal layouts
+ (NSArray *) equalLayouts:(NSArray *) views {
	BMFIXView *firstView = views.firstObject;
	return [self equalLayouts:views inParent:firstView.superview];
}

+ (NSArray *) equalLayouts:(NSArray *) views inParent:(BMFIXView *)parent {
	NSMutableArray *constraints = [NSMutableArray array];
	
	[constraints addObjectsFromArray:[self equalTops:views inParent:parent]];
	[constraints addObjectsFromArray:[self equalLefts:views inParent:parent]];
	[constraints addObjectsFromArray:[self equalRights:views inParent:parent]];
	[constraints addObjectsFromArray:[self equalBottoms:views inParent:parent]];
...
+ (NSArray *) equalCenters: (NSArray *)views inParent:(BMFIXView *)parent {
	NSArray *horizontal = [self equalHorizontalCenters:views inParent:parent];
	return [horizontal arrayByAddingObjectsFromArray:[self equalVerticalCenters:views inParent:parent]];
}

+ (NSArray *) equalHorizontalCenters: (NSArray *)views {
	BMFIXView *firstView = views.firstObject;
	return [self equalHorizontalCenters:views inParent:firstView.superview];
}
...
+ (NSArray *) equalVerticalCenters: (NSArray *)views {
	BMFIXView *firstView = views.firstObject;
	return [self equalVerticalCenters:views inParent:firstView.superview];
}
...
+ (void) setContentCompressionResistance:(BMFIXView *) view priority:(BMFLayoutPriority) priority axis:(BMFLayoutConstraintAxis) axis {
	...
	[view setContentCompressionResistancePriority:priority forOrientation:(NSLayoutConstraintOrientation)axis];
#endif
}
...
#else
	[view setContentHuggingPriority:priority forOrientation:(NSLayoutConstraintOrientation)axis];
#endif
