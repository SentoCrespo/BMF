...
//	[BMFAutoLayoutUtils constraint:@[ iconView ] toParent:iconContainerView margin:0];
	[BMFAutoLayoutUtils fill:iconContainerView with:iconView margin:0];

	[BMFAutoLayoutUtils constraint:@[ iconContainerView ] toParent:self margin:0];
	...
	//	[BMFAutoLayoutUtils fill:self with:iconContainerView margin:0];
	//	[BMFAutoLayoutUtils fillHorizontally:self withViews:@[ iconContainerView, label ] margin:0];
	//	[BMFAutoLayoutUtils distributeVertically:@[ iconContainerView, label ] inParent:self margin:0];
	
	[BMFAutoLayoutUtils fill:self with:iconContainerView margin:0];
	[BMFAutoLayoutUtils fillHorizontally:self withViews:@[ label ] margin:0];
	NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:iconView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:label attribute:NSLayoutAttributeTop multiplier:1 constant:0];
