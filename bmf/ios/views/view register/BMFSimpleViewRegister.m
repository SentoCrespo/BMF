...
//		NSString *kindString = [UICollectionView BMF_kindStringFromKind:info.kind];
		...
		if ([info.classOrUINib isKindOfClass:[UINib class]]) {
			[collectionView registerNib:info.classOrUINib forSupplementaryViewOfKind:info.kind withReuseIdentifier:info.viewId];
		}
		else if (class_isMetaClass(object_getClass(info.classOrUINib))) {
			[collectionView registerClass:info.classOrUINib forSupplementaryViewOfKind:info.kind withReuseIdentifier:info.viewId];
		}
	}];
}
