...
- (instancetype) initWithTitle:(NSString *)title message:(NSString *)message {
	...
	if (self) {
		...
		[[[self.BMF_delegate rac_signalForSelector:@selector(alertView:didDismissWithButtonIndex:) fromProtocol:@protocol(UIAlertViewDelegate)] reduceEach:^id(UIAlertView *alertView, NSNumber *buttonIndex) {
			return buttonIndex;
...
- (void) BMF_addButtonWithTitle:(NSString *) title actionBlock:(BMFActionBlock)block {
	BMFAssertReturn(title.length>0);
...
- (void) BMF_addCancelButtonWithTitle:(NSString *) title actionBlock:(BMFActionBlock)block {
	...
	@weakify(self);
	self.BMF_buttonsArray[self.cancelButtonIndex] = ^{
		@strongify(self);
		if (block) block(self);
	};
}
