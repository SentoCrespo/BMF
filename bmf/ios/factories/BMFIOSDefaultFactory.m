...
- (id) viewRegister:(NSString *)viewId kind:(NSString *)kind classOrNib:(id) classOrNib sender:(id) sender {
...
- (id<BMFDataSourceProtocol>) collectionViewDataSourceWithStore:(id<BMFDataReadProtocol>) store cellRegister:(id<BMFCellRegisterProtocol>)cellRegister animatedUpdates:(BOOL) animatedUpdates sender:(id) sender {
	BMFCollectionViewDataSource *dataSource = [[BMFCollectionViewDataSource alloc] initWithDataStore:store animatedUpdates:animatedUpdates];
...
- (id<BMFDataSourceProtocol>) collectionViewDataSourceWithStore:(id<BMFDataReadProtocol>) store cellIdentifier:(NSString *)cellIdentifier sender:(id) sender {
	...
	return [self collectionViewDataSourceWithStore:store cellRegister:cellRegister animatedUpdates:NO sender:sender];
}
...
- (id<BMFDataSourceProtocol>) collectionViewDataSourceWithStore:(id<BMFDataReadProtocol>) store cellClassOrNib:(id) classOrNib sender:(id) sender {
	return [self collectionViewDataSourceWithStore:store cellClassOrNib:classOrNib animatedUpdates:NO sender:sender];
}
...
- (id<BMFDataSourceProtocol>) collectionViewDataSourceWithStore:(id<BMFDataReadProtocol>) store cellRegisterBlock:(BMFCellRegisterCellsBlock) cellRegisterBlock cellIdBlock:(BMFCellIdBlock)cellIdBlock sender:(id) sender {
...
- (id<BMFDataSourceProtocol>) collectionViewDataSourceWithStore:(id<BMFDataReadProtocol>) store cellRegisterBlock:(BMFCellRegisterCellsBlock) cellRegisterBlock cellIdBlock:(BMFCellIdBlock)cellIdBlock animatedUpdates:(BOOL) animatedUpdates sender:(id) sender {
	
	id<BMFCellRegisterProtocol> cellRegister = nil;
	if (cellRegisterBlock) cellRegister = [self blockCellRegister:cellRegisterBlock cellIdBlock:cellIdBlock sender:sender];
	
	return [self collectionViewDataSourceWithStore:store cellRegister:cellRegister animatedUpdates:animatedUpdates sender:sender];
}
...
- (BMFMapViewController *) mapViewControllerWithStore:(id<BMFDataReadProtocol>) store sender:(id)sender {
	...
	vc.dataSource = (id)[self mapViewDataSourceWithStore:store sender:sender];
	BMFAssertReturnNil(!vc.dataSource || [vc.dataSource conformsToProtocol:@protocol(BMFDataSourceProtocol)]);
	return vc;
}

- (id<BMFUserMessagesPresenterProtocol>) defaultUserMessagesPresenter {
	return [BMFAlertViewUserMessagesPresenter new];
}
...
- (id<BMFDataReadProtocol>) dataSourceForView:(id)view dataStore:(id<BMFDataReadProtocol>)store cellClassOrNib:(id)classOrNib sender:(id)sender {
	return [self dataSourceForView:view dataStore:store cellClassOrNib:classOrNib animatedUpdates:NO sender:sender];
}

- (id<BMFDataReadProtocol>) dataSourceForView:(id)view dataStore:(id<BMFDataReadProtocol>)store cellClassOrNib:(id)classOrNib animatedUpdates:(BOOL)animatedUpdates sender:(id)sender {
	
	if ([view isKindOfClass:[UITableView class]]) {
		return [self tableViewDataSourceWithStore:store cellClassOrNib:classOrNib animatedUpdates:animatedUpdates sender:sender];
	}
	else if ([view isKindOfClass:[UICollectionView class]]) {
		return [self collectionViewDataSourceWithStore:store cellClassOrNib:classOrNib animatedUpdates:animatedUpdates sender:sender];
	}
	else if ([view isKindOfClass:[UICollectionView class]]) {
		return [self mapViewDataSourceWithStore:store sender:sender];
	}
	
	BMFThrowException(@"Couldn't find a valid data source for view: %@",view);
	return nil;
}
