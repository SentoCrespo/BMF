...
- (void) setValue:(id)value {
	[NSException raise:@"Value is not used for BMFActionSheetActivity" format:@"Use children activities instead"];
}
...
- (void) run:(BMFCompletionBlock)completionBlock {
	...
	if (!view) view = self.viewController.view;
	...
	if (self.showMode==BMFActionSheetActivityShowInView) {
		...
		BMFAssertReturn(self.name); /// The title of the action sheet is required, even if the docs don't say so. If not you get autolayout exceptions

		[actionSheet showFromRect:self.rect inView:view animated:self.showAnimated];
	}
	else {
